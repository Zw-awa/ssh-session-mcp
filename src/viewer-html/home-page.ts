...
function buildClientScript(debug: boolean, baseUrl: string, refreshMs: number): string {
  ...
  lines.push('if(diag.session&&diag.session.inputLock&&diag.session.inputLock!=="none")h+=" | Lock: "+e(diag.session.inputLock);if(diag.session&&diag.session.lockPolicy)h+=" | Policy: "+e(diag.session.lockPolicy);if(diag.session&&diag.session.userDraftActive)h+=" | Draft: active";h+="</div>";');
