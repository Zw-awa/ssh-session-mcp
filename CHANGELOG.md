...
### Fixed
- **Test suite stability**: Fixed intermittent test failures in viewer HTTP handler and WebSocket handler test suites

## [2.6.6] - 2026-04-22

### Fixed
- **Apache License copyright notice**: Updated LICENSE file template placeholder with correct copyright attribution
...
### Fixed
- **Viewer UI responsiveness**: Fixed CSS layout for narrow screens (<900px) with improved flexbox wrapping, overflow handling, and mobile-friendly headers
- **Lock mode JavaScript logic**: Corrected xterm browser terminal lock handling to treat "common" mode as unlocked (lock: 'none') instead of coercing it to user lock
- **Input actor separation**: Separated `getLockMode()` from `getInputActor()` functions to properly track UI lock state vs. input attribution

### Added
- **Viewer UI contract tests**: Added `test/viewer-html.test.ts` and `test/viewer-ui-contracts.test.ts` to verify responsive design and lock mode behavior
...
### Added
- **Promotion directory exclusion**: Added `Promotion/` to `.gitignore` and `.npmignore` for campaign material organization
- **Enhanced tool response fields**: Added `source`, `configResolution`, and `viewerEnabled` fields to `ssh-quick-connect` and `ssh-open-session` responses for better debugging

### Changed
- **Environment file loading**: Improved `.env` discovery to check both current working directory and module-relative paths
- **Documentation updates**: Added configuration discovery rules and auth schema guidance to README, README.zh-CN.md, and AI_AGENT_GUIDE.md
- **Configuration examples**: Enhanced `.env.example` with passwordEnv usage notes
